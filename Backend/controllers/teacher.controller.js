...
exports.getStudentStats = async (req, res) => {
  try {
    ...
    const acceptedRequests = await Request.findAll({
      where: { teacherId, status: 'accepted' },
      include: [{ model: User, as: 'student', attributes: ['id', 'username', 'purchasedHours', 'lastExamFailed'] }]
    });
    ...
    const stats = await Promise.all(acceptedRequests.map(async (request) => {
      ...
      return {
        ...
        isExamReady: totalHours >= requiredTotal && !successExam && !student.lastExamFailed,
        hasLicense: !!successExam,
        purchasedHours: extraHours,
        lastExamFailed: student.lastExamFailed === 1 || student.lastExamFailed === true
      };
    }));
