...
app.get("/auth/me", verifyToken, async (req, res) => {
  try {
    ...
    const isFailed = lastExam && lastExam.examResult === 'sikertelen';
...
app.get("/teacher/student-stats/:teacherId", verifyToken, async (req, res) => {
  try {
    ...
    console.log("Statisztika lekérése ehhez a tanárhoz:", teacherId);
    ...
    const stats = await Promise.all(acceptedRequests.map(async (request) => {
      ...
      const extraHours = (student && student.purchasedHours) ? student.purchasedHours : 0;
