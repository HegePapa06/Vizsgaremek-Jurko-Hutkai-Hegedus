  ...
  try {
    ...
    if (type === 'vizsga') {
      const existingExam = await Lesson.findOne({
        where: { studentId, type: 'vizsga', status: ['planned', 'pending_result'] }
      });
      if (existingExam) {
        return res.status(400).json({ message: "Hiba: Már van aktív vagy döntésre váró vizsgaidőpont!" });
      }

      if (totalHours < 30) {
        return res.status(400).json({ message: `Hiba: Még csak ${totalHours} óra van kész. 30 kell!` });
      }

      const failedExam = await Lesson.findOne({
        where: { studentId, type: 'vizsga', examResult: 'sikertelen' }
      });
      if (failedExam && !user.canRetakeExam) {
        return res.status(400).json({ message: "Hiba: Bukás után pótvizsga alkalmat kell vásárolni!" });
      }

      if (user.canRetakeExam) {
        user.canRetakeExam = false;
        await user.save();
      }

    } else {
      if (totalHours >= 30) {
        if (user.purchasedHours <= 0) {
          return res.status(400).json({ message: "A 30 óra betelt! Plusz vezetéshez vegyél órát a boltban." });
        }
        user.purchasedHours -= 1;
        await user.save();
      }
    }
...
async function updatePastLessons() {
  ...
  try {
    const lessonsToProcess = await Lesson.findAll({
      where: {
        status: 'planned',
        [Op.or]: [
          { date: { [Op.lt]: todayStr } },
    ...
    for (const lesson of lessonsToProcess) {
      if (lesson.type === 'vizsga') {
        if (!lesson.examResult) {
          await lesson.update({ status: 'pending_result' });
          console.log(`Vizsga várólistára került (ID: ${lesson.id})`);
        }
      } else {
        const previousCount = await Lesson.count({
          where: { studentId: lesson.studentId, status: 'completed', type: 'vezetés' }
        });
        await lesson.update({
          status: 'completed',
          lessonNumber: (previousCount + 1) * 2
        });
      }
    }
  } catch (err) {
...
app.get("/teacher/student-stats/:teacherId", verifyToken, async (req, res) => {
  try {
    ...
    const result = stats.filter(s => s !== null);
    console.log(`Statisztika küldése: ${result.length} diák adatai összegezve.`);
    res.json(result);
...
app.put("/lessons/:id", verifyToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { date, startTime, examResult, status, type } = req.body; 
    ...
    if (lesson.type === 'vizsga' && lesson.examResult && lesson.examResult.trim() !== "") {
      console.log("Módosítás megtagadva: A vizsga már le van zárva!");
      return res.status(403).json({ 
        message: "Ez a vizsga már lezárult, az eredménye nem módosítható!" 
      });
    }
...
app.put('/lessons/:id/result', verifyToken, async (req, res) => {
  const lessonId = req.params.id;
  ...
  try {
    ...
    if (!lesson) {
      return res.status(404).json({ message: "Nem található ilyen azonosítójú óra!" });
    }
    ...
    if (examResult === 'sikeres') {
      const student = await User.findByPk(lesson.studentId);
      if (student) {
        await student.update({ hasLicense: true });
      }
    }

    console.log(`Vizsga eredmény rögzítve (Sequelize) - ID: ${lessonId}, Eredmény: ${examResult}`);
    res.json({ message: "Sikeres mentés!", lesson });

  } catch (err) {
    console.error("Hiba a mentéskor:", err);
    res.status(500).json({ error: "Szerver hiba történt a mentés során." });
  }
});


app.post("/student/buy-item", verifyToken, async (req, res) => {
  try {
